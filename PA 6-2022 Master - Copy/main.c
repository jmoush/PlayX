...
//Buttons A-C on toy,M for mode and H is for volume. Use L if there is two buttons for volume control.
	if (PA2 == 0x04){Delay(500000);UART_OutChar('A');} 
	if (PA3 == 0x08){Delay(500000);UART_OutChar('B');}
	if (PA4 == 0x10){Delay(500000);UART_OutChar('C');}
	if (PA5 == 0x20){
	Delay(1);
	while(PA5 != 0x00){}
	...
	//if (PA7 == 0x80){Delay(500000);UART_OutChar('L');}
	}	
}
